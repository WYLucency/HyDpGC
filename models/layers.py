...
class HyperbolicGraphConvolution(torch.nn.Module):
    def __init__(self, in_features, out_features, curvature=1.0, with_bias=True):
        super().__init__()
    ...
    def forward(self, x, adj):
        # 双曲线性变换
        x = self.manifold.mobius_matvec(self.weight.t(), x)
        assert x.requires_grad, "mobius_matvec broke gradients"
        # 消息传递
        if isinstance(adj, SparseTensor):
            x = adj @ x
        else:
        ...
        assert x.requires_grad, "projx broke gradients"
        if self.bias is not None:
            x = x + self.bias
        return x
